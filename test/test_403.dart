...
void main() async {
  ...
  final res1 = await dio.get('https://open.spotify.com/track/0bnSXl0lY92annYcbgo1T2');
  final title1 = RegExp(r'<title>(.*?) - song and lyrics by (.*?) \| Spotify</title>').firstMatch(res1.data.toString());
  print('Track 1: ${title1?.group(1)} by ${title1?.group(2)}');
  
  final res2 = await dio.get('https://open.spotify.com/track/6FNYucvtKe0Qt6b23dYqHL');
  final title2 = RegExp(r'<title>(.*?) - song and lyrics by (.*?) \| Spotify</title>').firstMatch(res2.data.toString());
