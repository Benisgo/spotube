...
void main() async {
  ...
  try {
    ...
    final invUrl =
        'https://inv.thepixora.com/latest_version?id=${video.id.value}&itag=251&local=true';
    print('Testing $invUrl');

    final res = await dio.head(invUrl,
        options: Options(
          validateStatus: (_) => true,
          followRedirects:
              false, // Don't follow redirects to see if it actually proxies
        ));
    ...
    // What about Verome?
    final verUrl =
        'https://yt.omada.cafe/latest_version?id=${video.id.value}&itag=251&local=true';
    print('Testing $verUrl');
    final res2 = await dio.head(verUrl,
        options: Options(
          validateStatus: (_) => true,
